...
export default function TimeTable() {
  ...
  const { currentClasses, currentSemester, startDate } = useSelector(
    state => state.course
  );
  ...
  useEffect(() => {
    ...
    // 计算该周有效课程
    dispatch(
      getAvailableCourse({ currentClasses: currentClasses, dates: _dates })
    );
  ...
  const handleClickFab = () => {
    if (handleLoginStatus()) Taro.navigateTo({ url: "/pages/add-class/index" });
  };
  ...
  return (
    <View className="container">
      ...
      <View className="time-course-view">
        ...
        <TimeBar
          timeZoneStart={
            classMode === "中国境内" ? (AU_TIME_ZONE === "AEDT" ? 5 : 6) : 8
          }
        />
        {/* 课程表主体 */}
        <CourseTable />
      ...
      <AtFab
        className="auto-import-fab"
        size="small"
        onClick={() => dispatch(setShowAutoImport(true))}
      >
        <AtIcon
          prefixClass="icon"
          value="ilink-upload"
