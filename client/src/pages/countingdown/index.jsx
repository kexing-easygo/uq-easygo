...
export default function CountingDown() {
  ...
  const { completeCountDown, incompleteCountDown } = splitCountDown(
    sortCountDown(userCountDown, classMode)
  );
          ...
          <View className="countdown-container">
            ...
            {Object.entries(incompleteCountDown).map(entry => (
              <>
                ...
                />
                <View>
                  {entry[1].map(assignment => (
                    <SwiperItem assignmentInfo={assignment} />
                  ))}
                </View>
              </>
            ))}
