...
export default function OwnReview(props) {
  ...
  return (
    <View>
      ...
      <AtCard
        note={"编辑于" + review.postDate + " " + review.postTime}
        title=""
        className="own-review-card"
      >
        {/* <View className='own-review-background' style={{backgroundColor:color}} */}
        <View
          className="own-review-background"
      ...
      <View className="own-review-heart">
        <View className="own-icon-view">
          ...
          <AtModal
            isOpened={showCancelModal}
            onClose={() => changeCancelModalState(false)}
          >
            <AtModalHeader>温馨提示</AtModalHeader>
            <AtModalContent>请问确定要删除这个评价吗?</AtModalContent>
            <AtModalAction>
              <Button onClick={() => changeCancelModalState(false)}>
                取消
              </Button>
              <Button
                onClick={() => {
        ...
        <OthersReviewAction
          reviewsCount={review.numOfComments}
