...
async function appendUserClasses(event) {
  ...
  const newClassesInfo = await Promise.all(
    classes.map(cl => cl.subscribed ? cl : __fetchClassById(timetableCollection, cl._id))
  );
...
function _analyseData(calender) {
  ...
  calender.forEach((element) => {
    const {
      ...
      subject_code,
      activity_group_code
    } = getDetailedClassId(element.description.value)
    const {
      dtstart,
      dtend
    } = element
...
async function autoAddTimetable(openid, timetableLink, branch) {
  return requestPromise(timetableLink)
    .then(async (req) => {
      ...
      const currentSelectedCourses = await getSelectedCourses(openid, branch + MAIN_USER_SUFFIX)
      // 覆盖本学期已经添加的所有课程
      currentSelectedCourses[CURRENT_SEMESTER] = res
      // console.log(currentSelectedCourses)
      const finalRes = await db.collection(branch + MAIN_USER_SUFFIX)
        .where({
          _openid: openid
        })
        .update({
